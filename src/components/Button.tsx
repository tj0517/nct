...
export default function Button({
  ...
  const sizeClasses =
    size === "small"
      ...
      : "font-light text-lg md:text-2xl h-auto py-4 md:h-[75px] w-full lg:w-auto md:px-10";
  ...
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className={`group relative font-fraunces ${sizeClasses} rounded-bl-[25px] rounded-tr-[25px] border flex items-center justify-center overflow-hidden ${disabled ? "cursor-not-allowed" : "cursor-pointer"} ${variantClasses[variant]} ${className}`}
    >
