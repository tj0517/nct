...
export default function BookingModal({ isOpen, onClose, dict }: BookingModalProps) {
  // Body scroll lock
  useEffect(() => {
    if (isOpen) {
        ...
        <div className="flex gap-3">
          <label className="flex-1 flex items-center gap-3 bg-main-bg/90 rounded-bl-[25px] rounded-tr-[25px] px-5 md:px-6 py-3 md:py-4 cursor-pointer has-[:checked]:bg-main-bg has-[:checked]:ring-2 has-[:checked]:ring-main-bg transition-colors">
            <input
              ...
              defaultChecked
            />
            <span className="font-inter text-sm md:text-base text-main">{dict.inPerson}</span>
          </label>
          <label className="flex-1 flex items-center gap-3 bg-main-bg/90 rounded-bl-[25px] rounded-tr-[25px] px-5 md:px-6 py-3 md:py-4 cursor-pointer has-[:checked]:bg-main-bg has-[:checked]:ring-2 has-[:checked]:ring-main-bg transition-colors">
            <input
              type="radio"
              ...
              className="accent-main-bg w-4 h-4"
            />
            <span className="font-inter text-sm md:text-base text-main">{dict.online}</span>
          </label>
        </div>
